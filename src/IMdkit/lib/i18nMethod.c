...
static Bool GetInputStyles (Xi18n i18n_core, XIMStyles **p_style) {
    ...
    if ((*p_style = (XIMStyles *) malloc (sizeof (XIMStyles) + p->count_styles * sizeof (XIMStyle))) == NULL) {
        return False;
    }
    /*endif*/
...
static Bool GetOnOffKeys (Xi18n i18n_core, long mask, XIMTriggerKeys **p_key) {
    ...
    if (mask & I18N_ON_KEYS)
        ...
        p = &address->off_keys;
    /*endif*/
    if ((*p_key = (XIMTriggerKeys *) malloc (sizeof (XIMTriggerKeys) + p->count_keys * sizeof (XIMTriggerKey))) == NULL) {
        return False;
    }
    /*endif*/
...
static Bool GetEncodings (Xi18n i18n_core, XIMEncodings **p_encoding) {
    ...
    if ((*p_encoding = (XIMEncodings *) malloc (sizeof (XIMEncodings) + p->count_encodings * sizeof (XIMEncoding))) == NULL) {
        return False;
    }
    /*endif*/
    ...
    for (i = 0; i < (int) p->count_encodings; i++) {
        (*p_encoding)->supported_encodings[i] = (char *) malloc (strlen (p->supported_encodings[i]) + 1);
    ...
    if (mode == I18N_OPEN || mode == I18N_SET) {
        for (p = args; p->name != NULL; p++) {
            if (strcmp (p->name, IMLocale) == 0) {
                ...
                if (address->encoding_list.supported_encodings == (XIMEncoding *) NULL) {
                    return IMEncodingList;
                }
                /*endif*/
...
static Bool WaitXSelectionRequest (Display *dpy,
    ...
    if (((XSelectionRequestEvent *) ev)->selection == i18n_core->address.selection) {
...
static void EventToWireEvent (XEvent *ev, xEvent *event, CARD16 *serial, Bool byte_swap) {
    FrameMgr fm;
    ...
    *serial = (CARD16) (ev->xany.serial >> 16);
    ...
    case KeyRelease: {
        ...
        c32 = (CARD32) kev->time;
        FrameMgrPutToken (fm, c32);
        c32 = (CARD32) kev->root;
        FrameMgrPutToken (fm, c32);
        c32 = (CARD32) kev->window;
        FrameMgrPutToken (fm, c32);
        c32 = (CARD32) kev->subwindow;
        FrameMgrPutToken (fm, c32);
        c16 = (CARD16) kev->x_root;
        FrameMgrPutToken (fm, c16);
        c16 = (CARD16) kev->y_root;
        FrameMgrPutToken (fm, c16);
        c16 = (CARD16) kev->x;
        FrameMgrPutToken (fm, c16);
        c16 = (CARD16) kev->y;
        FrameMgrPutToken (fm, c16);
        c16 = (CARD16) kev->state;
        FrameMgrPutToken (fm, c16);
        b = (BYTE) kev->same_screen;
        FrameMgrPutToken (fm, b);
    } break;
    default:
        /* create FrameMgr */
